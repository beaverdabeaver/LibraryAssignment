...
public class BookDetailController implements Initializable{
	...
	@FXML private Button additionalButtonID;
	@FXML private Button generateBooks;
	...
	void saveButtonClicked(MouseEvent event){
		...
		if(!book.isValidTitle(book.getTitle())) {
			...
			book.save();
			try {
				check = book.getAuthors();
			} catch (Exception e1) {
				// TODO Auto-generated catch block
				logger.error("this is a new book");
				//e1.printStackTrace();
			}
			if(check == null || check.isEmpty()){
				logger.error("No authors selected for book!");
				//AlertHelper.showWarningMessage("SAVE ERROR", "Book has no Author", "Please add one in the Additional Info menu");
				try {
					additionalButtonClicked(null);
				} catch (Exception e) {
					// TODO Auto-generated catch block
					e.printStackTrace();
				}
	...
	@FXML
	void additionalButtonClicked(MouseEvent event) throws Exception {
